...
exports.checkServiceAvailability = async (req, res) => {
  try {
    ...
    // ============================================
    // 2. Find nearby washermen
    // ============================================
    // This is ONLY the maximum database search area.
    // Actual service availability is checked using
    // washerman.range below.

    const MAX_SEARCH_DISTANCE = 50000; // 50 KM in meters
    ...
    // ============================================
    // 3. Prepare response arrays
    // ============================================

    const availableServices = [];
    const outOfRangeWashermen = [];
    const noProductWashermen = [];

    // ============================================
    // 4. Check every washerman
    // ============================================

    for (const washerman of washermen) {
      // --------------------------------------------
      // Validate washerman location
      // --------------------------------------------

      if (
        !washerman.location ||
        !Array.isArray(washerman.location.coordinates) ||
        washerman.location.coordinates.length !== 2
      ) {
        continue;
      }

      const washermanLng = Number(
        washerman.location.coordinates[0]
      );

      const washermanLat = Number(
        washerman.location.coordinates[1]
      );

      // Validate washerman coordinates
      if (
        !Number.isFinite(washermanLat) ||
        !Number.isFinite(washermanLng) ||
        (washermanLat === 0 && washermanLng === 0)
      ) {
        continue;
      }
      ...
      // --------------------------------------------
      // Get washerman service range
      // --------------------------------------------

      const washermanRange =
        Number(washerman.range) > 0
          ? Number(washerman.range)
          : 500;
      ...
      console.log(
        `Washerman: ${washerman.name}`
      );

      console.log(
        `Washerman Location: ${washermanLat}, ${washermanLng}`
      );
      ...
      console.log(
        `Service Range: ${washermanRange} meters`
      );

      console.log(
        `Inside Range: ${
          distanceInMeters <= washermanRange
        }`
      );
      ...
      // ============================================
      // 5. IMPORTANT RANGE CHECK
      // ============================================
      // If customer is outside washerman's range,
      // NEVER add this washerman's services.
      ...
      // ============================================
      // 6. Customer is inside washerman's range
      // ============================================

      let hasValidProduct = false;
      // ============================================
// DEBUG: Check washerman's products
// ============================================



console.log("====================================");
console.log("Washerman:", washerman.name);
console.log("Products:", washerman.products);
console.log("Product Count:", washerman.products?.length);
console.log("====================================");
...
for (const product of washerman.products || []) {
  ...
  if (
    ...
    console.log("SKIPPED: Product has no options");
    continue;
  }

  console.log("PRODUCT ADDED TO AVAILABLE SERVICES");
  ...
  availableServices.push({
    ...
    washerman: {
      ...
      // ============================================
      // 7. Washerman is in range but has no products
      // ============================================

      if (!hasValidProduct) {
        noProductWashermen.push({
          _id: washerman._id,
          name: washerman.name,
          distance: distanceInMeters,
          range: washermanRange,
    ...
    // ============================================
    // 9. Get unique washermen with services
    // ============================================

    const uniqueWashermen = [
      ...new Set(
        availableServices.map(
          (service) =>
            service.washerman._id.toString()
        )
      ),
    ];

    // ============================================
    // 10. Sort services by distance
    // ============================================

    availableServices.sort(
      (a, b) =>
        a.washerman.distance -
        b.washerman.distance
    );
    ...
    const response = {
      ...
      customerLocation: {
        ...
        address:
          ...
          `${customerLat.toFixed(
            6
          )}, ${customerLng.toFixed(6)}`,
      },

      availability: {
        // Is there at least one service?
        servicesAvailable:
        ...
        // Total washermen found within 50 KM
        totalWashermen:
          washermen.length,

        // Washermen who are inside their
        // configured service range
        inRangeWashermen:
          uniqueWashermen.length,

        // Washermen outside their range
        outOfRangeWashermen:
          outOfRangeWashermen.length,
      },

      // ONLY in-range washerman services
      services:
        availableServices,

      message:
        availableServices.length > 0
          ? `Found ${availableServices.length} services from ${uniqueWashermen.length} washermen in your area`
          : outOfRangeWashermen.length > 0
          ? `No services available in your area. Found ${outOfRangeWashermen.length} washermen, but they are outside their service range.`
          : `No washermen found near your location.`,
    };

    // ============================================
    // 12. Closest washerman suggestion
    // ============================================
    ...
    if (
      ...
      // Find closest washerman
      const closestWasherman =
        [...outOfRangeWashermen].sort(
          (a, b) =>
            a.distance - b.distance
        )[0];

      response.suggestion =
        `The closest washerman (${closestWasherman.name}) is ` +
        `${closestWasherman.distance}m away, ` +
        `but their service range is ` +
        `${closestWasherman.range}m.`;
    }
    ...
    return res.status(500).json({
      ...
      message:
        "Server error while checking service availability",
    });
  }
};
